...
def main():
    try:
        ...
        nudge = (
            ...
            "security-critical, final review/verification, needs live tools the local model can't "
            "drive) OR plain cost-benefit: for small or one-off work where spin-up/briefing overhead "
            ...
            f"(and effort) with `{resolver}`; dispatch via curl / bin/librarian-dispatch.py. The smell "
            "is reflexively doing BULK/mechanical work yourself on the paid model — not skipping "
            "offload on trivia. Full guidance: the offload-to-local skill."
        )
        print(json.dumps({
            "hookSpecificOutput": {
