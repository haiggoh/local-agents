...
**Current reconstructed version:** **v0.9.0**, pending named-session functional validation
...
## v0.10.0 — Session management and attachment accounting
...
The installed script corresponds to **reconstructed v0.9.0**.

Installation and static validation have passed. The immediate next step is to complete the named-session functional test across two separate processes and verify the JSON file’s `0600` permissions.

After that validation, the recommended next milestone is **v0.10.0: session management and attachment accounting**, followed by test/release infrastructure rather than another large dispatcher feature.